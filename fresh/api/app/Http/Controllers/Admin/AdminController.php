...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $total_male = User::where('user_gender','male')->count();
        $total_female = User::where('user_gender','female')->count();
        $today_user = User::where('created_at', '>=', Carbon::today())->count();
        $last7days = User::where('created_at', '>=', Carbon::now()->subDays(7))->count();
        $user = User::orderBy('id','desc')->limit('8')->get();
         
         
        return view('admin/dashboard',compact('total_user','total_male','total_female','today_user','last7days','user'));
    }
    ...
    public function reported_user(Request $request){ 
        $users = ReportUser::select("report_users.*","reportedby.full_name as reportedby_name","reportedby.email as reportedby_email","reportedby.user_phone as reportedby_phone","reportedto.full_name as reportedto_name","reportedto.email as reportedto_email","reportedto.user_phone as reportedto_phone")
        ->join("users as reportedby","reportedby.id","=","report_users.reported_by")
        ->join("users as reportedto","reportedto.id","=","report_users.report_user_id") 
        ->get();
        
     //$users = ReportUser::with('ruser','reported_by')->get();
      return view('admin/reported_userlist',compact('users')); 
    }
